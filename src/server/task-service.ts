...
export class TaskService implements Dispatcher {
  ...
  async retry(id: string, input: RetryTaskInput = {}) {
    ...
    if (!['retry', 'fix', 'replan'].includes(mode)) throw new DomainError('Unknown recovery mode.');
    ...
    const phase = mode === 'replan' ? 'planning' : mode === 'fix' ? 'building' : task.phase === 'verification' || task.phase === 'building' ? task.phase : 'planning';
    ...
    const saved = await this.change(task, { status: 'todo', phase, error: undefined, runId: undefined, sessionId: undefined, plans, recovery }, mode === 'replan' ? 'Replanning requested. A new RFC requires owner approval before implementation.' : mode === 'fix' ? 'Implementation remediation queued within the approved RFC.' : 'Retry queued.');
