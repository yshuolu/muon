...
export const verificationSchema = z.object({ summary: z.string().min(1).max(30_000), evidence: z.array(evidenceSchema).min(1).max(100) });
...
Use kind:"group" for organizational parents. Groups run no agent and complete only when their nonempty subtasks and dependencies are Done. Canceled subtasks do not count as Done; detach them with parentId:null only when requested. Nested groups are supported.
Tasks select exactly one workflow: {"kind":"brainstorm"}, {"kind":"research"}, or {"kind":"develop"}, supplied in the creation body's workflow field (or CLI --workflow). Omitted workflow means Develop. Brainstorm and Research each run one read-only agent session and save ideas or findings; they do not require a repository, an RFC, or coding verification. Use Brainstorm for exploring possibilities and Research for investigating questions. Develop is one workflow composed of Plan, Build, and Verify sessions. Its Plan result requires exact owner RFC approval before Build. Only the owner can supply the optional approvedPlan parameter to start a new Develop task at Build; the chief cannot reuse approvals. All sessions receive the same system instruction envelope, and the task plane coordinates their results. Prerequisite code changes live in separate worktrees; use blockedByIds for a Develop integration task rather than assuming changes are merged. Research and brainstorming dependencies supply saved results instead of Git patches.
...
Blocked task recovery: resume continues the current logical session's saved provider conversation when available; retry repeats that session with a fresh provider conversation; fix returns a Develop task to Build within the approved RFC; replan replaces the RFC and requires new owner approval. Brainstorm and Research support retry and resume. The chief cannot approve RFCs, submit owner reviews, mark tasks Done, change settings, or clear the owner's attention. These restrictions are enforced by the API. Never attempt to bypass them.
