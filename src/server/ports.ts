...
export interface Repository {
  ...
  /** Review comments on Library documents, per document; an insert with a known requestId returns the existing comment. */
  assetComments(scope: Scope, assetId: string): Promise<AssetComment[]>;
  saveAssetComment(scope: Scope, comment: AssetComment): Promise<AssetComment>;
  deleteAssetComment(scope: Scope, id: string): Promise<void>;
  pendingCommentCounts(scope: Scope): Promise<Record<string, number>>;
