...
const identity = new LocalIdentityProvider().currentScope();
...
const server = serve({ fetch: app.fetch, hostname: '127.0.0.1', port }, () => {
  void (async () => {
    await registry.load();
    if (demo) await seedDemo(registry.resolve(undefined), repository);
    ready = true;
    console.log(`Muon ${demo ? 'demo ' : ''}is ready at http://127.0.0.1:${port}`);
  })().catch(async error => { console.error(error); ready = false; server.close(); await registry.stopAll(); await releaseInstance(); repository.close(); process.exitCode = 1; });
});
