...
import { ArrowLeft, CheckCheck, ChevronRight, Code2, File, FileText, GitBranch, Layers3, Pencil, Plus, X } from 'lucide-react';
...
import { taskWorkflow, WORKFLOW_LABELS } from '../../shared/workflows';
...
import { taskTabs, type TaskTab } from '../lib/task-workflow';
...
export function TaskDetail({ task, snapshot, onClose, onRefresh, onSelect, onSubtask, backLabel }: { task: Task; snapshot: AppSnapshot; onClose: () => void; onRefresh: () => void; onSelect: (task: Task) => void; onSubtask: (task: Task) => void; backLabel: string }) {
  ...
  const isDevelop = workflow.kind === 'develop';
  const output = task.outputs?.at(-1);
  const finalResult = output?.content ?? task.summary;
  const availableTabs = taskTabs(task);
  const [selectedTab, setTab] = useState<TaskTab>(task.kind === 'group' ? 'overview' : task.phase === 'plan_review' ? 'plan' : task.status === 'done' && isDevelop ? 'evidence' : 'overview');
  const tab = availableTabs.includes(selectedTab) ? selectedTab : 'overview';
  ...
  const unstartedApprovedPlan = !isGroup && !!workflow.params?.approvedPlan && !task.runs?.length && !task.runId && ['backlog', 'todo'].includes(task.status);
  const canEditMetadata = canEdit || unstartedApprovedPlan;
  const canCancel = !['done', 'canceled'].includes(task.status);
  const footerNote = isGroup ? (task.status === 'done' ? 'All subtasks completed' : `${subtasks.filter(child => child.status === 'done').length} of ${subtasks.length} subtasks completed`) : task.status === 'todo' ? waiting[0] : task.status === 'done' ? (isDevelop ? task.worktree ? 'Verification complete · Branch retained' : 'Verification complete' : 'Outcome saved')
    ...
    : snapshot.settings.dispatcherEnabled ? 'Waiting for an agent' : 'Dispatch paused';
  ...
  return <section className="task-detail" aria-label={`${task.identifier}: ${task.title}`}>
    ...
    <div className={`detail-scroll ${tab === 'plan' && plan && task.status !== 'blocked' ? 'plan-review-workspace' : ''}`}>
      ...
      {!isGroup && <WorkflowRail task={task} />}
      ...
      }}>{([{ id: 'overview', label: 'Overview', count: 0 }, { id: 'plan', label: 'Plan', count: task.plans.length }, { id: 'evidence', label: 'Evidence', count: task.evidence.length }, { id: 'files', label: 'Files', count: task.changedFiles.length }] as const).filter(item => availableTabs.includes(item.id)).map(item => <button key={item.id} id={`tab-${item.id}`} aria-controls={`panel-${item.id}`} role="tab" tabIndex={tab === item.id ? 0 : -1} aria-selected={tab === item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}>{item.label}{item.id === 'plan' && latestPlan?.status === 'pending' ? <i className="review-dot" /> : item.count > 0 ? <span>{item.count}</span> : null}</button>)}</div>
      <div className={`detail-tab-content ${tab === 'plan' && plan ? 'plan-tab-content' : ''}`} id={`panel-${tab}`} aria-labelledby={`tab-${tab}`} role="tabpanel">
        {tab === 'overview' && <>
          <div className="overview-grid"><div className="overview-content">{finalResult && <div className={`result-summary ${isDevelop ? '' : 'saved-output'}`}><div className="section-label"><CheckCheck size={15} />{isGroup ? 'Group result' : output?.kind === 'ideas' ? 'Ideas' : output?.kind === 'report' ? 'Research findings' : 'Agent’s final result'}</div><Markdown>{finalResult}</Markdown></div>}<div className="section-label">Description</div>{task.description ? <Markdown>{task.description}</Markdown> : <p className="muted">No description yet. Add context to help your agent get started.</p>}
            ...
            {subtasks.length ? <><div className="subtask-progress-note">{isGroup ? 'This group completes when all subtasks finish successfully. Each task follows its selected workflow.' : isDevelop ? 'This task starts after its subtasks finish. Its RFC covers the remaining integration work.' : 'This task starts after its subtasks finish and uses their saved outcomes.'}</div><div className="subtask-list">{subtasks.map(child => <button key={child.id} onClick={() => onSelect(child)}><StatusIcon status={child.status} /><span className="task-identifier">{child.identifier}</span><span>{child.title}</span><ChevronRight size={13} /></button>)}</div></> : <div className="subtask-empty"><Layers3 size={16} /><span>{canEdit ? 'Break this work into smaller tasks.' : 'This task has no subtasks.'}</span></div>}
