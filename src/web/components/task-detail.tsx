...
export function TaskDetail({ task, snapshot, onClose, onRefresh, onSelect, onSubtask, backLabel }: { task: Task; snapshot: AppSnapshot; onClose: () => void; onRefresh: () => void; onSelect: (task: Task) => void; onSubtask: (task: Task) => void; backLabel: string }) {
  ...
  const plan = task.plans.find(p => p.id === planVersion) || latestPlan;
  ...
  return <section className="task-detail" aria-label={`${task.identifier}: ${task.title}`}>
    ...
    <div className="detail-scroll">
      ...
      {error && <p className="form-error detail-error" role="alert">{error}</p>}
      ...
      <div className="detail-tab-content" id={`panel-${tab}`} aria-labelledby={`tab-${tab}`} role="tabpanel">
        {tab === 'overview' && <>
        ...
        {tab === 'plan' && (!plan ? <EmptyState icon={<FileText size={26} />} title="Every good build starts with a plan" description={task.status === 'backlog' ? 'Move this task to Todo when you’re ready. Your agent will prepare an RFC for your approval.' : 'Your agent’s RFC will appear here when planning is complete. You review it before any building begins.'} /> : <div className="plan-content"><div className="plan-document-header"><div><span className="document-icon"><FileText size={20} /></span><div><h3>Request for comments</h3><p>{plan.status === 'approved' ? 'Approved by you' : plan.status === 'changes_requested' ? 'Changes requested' : 'Ready for your review'} · {relativeTime(plan.createdAt)}</p></div></div><select aria-label="Plan version" value={plan.id} onChange={e => setPlanVersion(e.target.value)}>{task.plans.map(p => <option key={p.id} value={p.id}>Version {p.version}{p.id === latestPlan?.id ? ' · latest' : ''}</option>)}</select></div>{plan.feedback && <div className="plan-feedback"><strong>Your feedback</strong><p>{plan.feedback}</p></div>}
          ...
          {plan.status === 'pending' && plan.id === latestPlan?.id && task.phase === 'plan_review' && <div className="plan-approval"><div><span className="approval-icon"><FileCheck2 size={19} /></span><span><strong>Your approval moves this forward</strong><small>The agent will build and verify this plan.</small></span></div>{requestChanges ? <form className="plan-feedback-form" onSubmit={async e => { e.preventDefault(); if (await mutate(`/tasks/${task.id}/request-changes`, 'POST', { planId: plan.id, feedback: feedback.trim() })) { setRequestChanges(false); setFeedback(''); } }}><label className="sr-only" htmlFor="plan-feedback">Requested changes</label><textarea id="plan-feedback" placeholder="What should change in the plan?" value={feedback} onChange={e => setFeedback(e.target.value)} rows={3} autoFocus required /><div><Button variant="ghost" type="button" onClick={() => setRequestChanges(false)}>Cancel</Button><Button disabled={busy || !feedback.trim()} type="submit">{busy ? 'Sending…' : 'Send feedback'}</Button></div></form> : <div className="approval-actions"><Button variant="secondary" onClick={() => setRequestChanges(true)} disabled={busy}>Request changes</Button><Button onClick={() => void mutate(`/tasks/${task.id}/approve`, 'POST', { planId: plan.id })} disabled={busy}><Check size={15} />{busy ? 'Approving…' : 'Approve plan'}</Button></div>}</div>}
          {plan.status === 'approved' && <div className="plan-approved"><CheckCircleIcon />Plan approved{plan.reviewedAt ? ` · ${relativeTime(plan.reviewedAt)}` : ''}</div>}
        </div>)}
...
function CheckCircleIcon() { return <span className="approved-check"><Check size={12} /></span>; }
