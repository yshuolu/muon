...
export function TaskDetail({ task, snapshot, onClose, onRefresh, onSelect, onSubtask, onOpenLibrary, backLabel }: { task: Task; snapshot: AppSnapshot; onClose: () => void; onRefresh: () => void; onSelect: (task: Task) => void; onSubtask: (task: Task) => void; onOpenLibrary?: (assetId: string | null) => void; backLabel: string }) {
  ...
  return <section className="task-detail" aria-label={`${task.identifier}: ${task.title}`}>
    ...
    <Dialog open={editing} onOpenChange={setEditing} title="Edit task" description="Keep the task’s goals and acceptance criteria clear."><form className="edit-task-form" onSubmit={async e => { e.preventDefault(); if (await update({ title: title.trim(), description, labels: [...new Set(labels.split(',').map(value => value.trim()).filter(Boolean))], parentId, blockedByIds })) setEditing(false); }}><label>Title<input value={title} onChange={e => setTitle(e.target.value)} required maxLength={240} /></label><label>Description<MentionTextarea value={description} onChange={setDescription} rows={7} maxLength={30000} placement="below" /></label><label>Labels<input value={labels} onChange={event => setLabels(event.target.value)} placeholder="frontend, improvement" /></label><TaskRelations tasks={snapshot.tasks} currentId={task.id} parentId={parentId} onParentChange={setParentId} blockedByIds={blockedByIds} onDependenciesChange={setBlockedByIds} />{error && <p role="alert" className="form-error">{error}</p>}<div className="dialog-footer"><Button variant="ghost" type="button" onClick={() => setEditing(false)}>Cancel</Button><Button type="submit" disabled={busy || !title.trim()}>{busy ? 'Saving…' : 'Save task'}</Button></div></form></Dialog>
  </section>;
}
