...
export function ChiefView({ snapshot, onRefresh, onSelect }: { snapshot: AppSnapshot; onRefresh: () => void; onSelect: (task: Task) => void }) {
  ...
  const [workingMessage, setWorkingMessage] = useState('Chief of staff is working on it…');
  ...
  useEffect(() => {
    const messages = ['Reviewing your workspace…', 'Thinking through next steps…', 'Organizing the work…', 'Preparing your response…'];
    if (!snapshot.runtime.chiefRunning || chiefQueued) { setWorkingMessage('Chief of staff is working on it…'); return; }
    setWorkingMessage(messages[0]);
    let index = 0;
    const timer = setInterval(() => { index = (index + 1) % messages.length; setWorkingMessage(messages[index]); }, 2600);
    return () => clearInterval(timer);
  }, [chiefQueued, snapshot.runtime.chiefRunning]);
