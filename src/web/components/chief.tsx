...
export function ChiefView({ snapshot, onRefresh, onSelect }: { snapshot: AppSnapshot; onRefresh: () => void; onSelect: (task: Task) => void }) {
  ...
  const modelOptions = [...new Set([chiefConfig.model, 'opus', 'sonnet', 'haiku'])];
  ...
  return <div className="chief-view"><div ref={conversation} className="chief-conversation">
    ...
    <Dialog open={modelOpen} onOpenChange={open => { if (!open && !savingModel) closeModel(); }} title="Chief of staff model" description="Choose a Claude model for this workspace’s Chief of staff.">
      <form className="settings-form" onSubmit={event => { event.preventDefault(); void saveModel(modelDraft.trim()); }}>
        <label><span id="chief-model-label">Model</span><input aria-labelledby="chief-model-label" aria-describedby="chief-model-hint" value={modelDraft} onChange={event => setModelDraft(event.target.value)} list="chief-model-options" required maxLength={200} autoComplete="off" autoFocus spellCheck={false} disabled={modelDisabled} /><span id="chief-model-hint" className="field-hint">Choose a suggestion or enter a model ID available to your Claude Code account.</span></label>
        <datalist id="chief-model-options">{modelOptions.map(option => <option key={option} value={option} />)}</datalist>
