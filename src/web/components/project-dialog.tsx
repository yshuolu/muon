...
export function ProjectDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: (project: Project) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const project = await api<Project>('/projects', 'POST', { name: name.trim(), repositoryPath: repositoryPath.trim(), ...(identifier.trim() ? { identifier: identifier.trim().toUpperCase() } : {}), ...(initializeRepository ? { initializeRepository: true } : {}) });
      onOpenChange(false);
      onCreated(project);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not add this project.'); }
    finally { setBusy(false); }
  }
  return <Dialog open={open} onOpenChange={onOpenChange} title="Add project" description="Point Muon at a project folder. Each project keeps its own tasks, agents, chief of staff, settings, and library.">
