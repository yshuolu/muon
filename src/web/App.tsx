...
import { LibraryDocument, LibraryView } from './components/library';
...
export function App() {
  ...
  // Planning chats are saved; leaving one keeps it under History until it is discarded or turned into a task.
  const closePlanningChat = useCallback(() => { navigate('tasks'); }, [navigate]);
  ...
  const documentOpen = view === 'library' && libraryAssetId !== null;
  // A task or a library document opens over the workspace; the page behind it is inert while it is open.
  const overlayOpen = !!selected || documentOpen;
  ...
  return <div className={`app-shell${compactViewport ? ' viewport-compact' : ''}`} style={viewportStyle}>
    ...
    <main className="workspace-main">
      ...
      {!compactPlanningChat && <div inert={overlayOpen} className="page-heading"><div><div className="page-heading-eyebrow"><span className="project-icon">{snapshot.project.name.slice(0, 1).toUpperCase()}</span>{snapshot.project.name}<span className="eyebrow-divider">/</span>{snapshot.project.identifier}</div><h1>{title}{view === 'attention' && unread > 0 && <span className="title-count">{unread}</span>}</h1><p>{view === 'tasks' ? 'A clear path from idea to verified work.' : view === 'attention' ? 'The decisions that keep your work moving.' : view === 'library' ? 'Documentation, reference notes, and every retained file in one place.' : 'A thoughtful partner for everything you’re building.'}</p></div><div className="page-actions">{view === 'tasks' && <div className="layout-toggle" aria-label="Task layout"><button aria-label="List view" aria-pressed={layout === 'list'} className={layout === 'list' ? 'active' : ''} onClick={() => setLayout('list')}><List size={15} /></button><button aria-label="Board view" aria-pressed={layout === 'board'} className={layout === 'board' ? 'active' : ''} onClick={() => setLayout('board')}><LayoutGrid size={14} /></button></div>}{view !== 'library' && <Button onClick={() => void openCreate()} disabled={creatingChat}><Plus size={15} />New task</Button>}</div></div>}
      {!snapshot.project.repositoryPath && <div inert={overlayOpen} className="setup-banner"><span className="setup-banner-icon"><FolderGit2 size={18} /></span><div><strong>Connect your repository</strong><p>Choose a local Git repository so your agents know where to work.</p></div><Button variant="secondary" size="sm" onClick={() => setSettings(true)}>Choose repository<ArrowUpRight size={13} /></Button></div>}
      {view === 'tasks' && <div inert={overlayOpen} className="workspace-stats"><div><span className="stat-icon active"><CircleDot size={15} /></span><span>In progress</span><strong>{running}</strong></div><button onClick={() => go('attention')}><span className="stat-icon review"><FileReviewIcon /></span><span>Awaiting review</span><strong>{reviews}</strong>{reviews > 0 && <ArrowUpRight size={13} />}</button><div><span className="stat-icon queued"><ArrowDownToLine size={15} /></span><span>In queue</span><strong>{todo}</strong></div><div><span className="stat-icon done"><Check size={15} /></span><span>Completed</span><strong>{done}</strong></div></div>}
      <div inert={overlayOpen} className={`page-content page-${view}`}>{view === 'tasks' ? <TaskList snapshot={snapshot} layout={layout} onSelect={selectTask} selectedId={selectedId || undefined} onCreate={() => void openCreate()} /> : view === 'attention' ? <AttentionView snapshot={snapshot} onSelect={selectTask} onRead={readAttention} /> : view === 'chief' ? <ChiefView snapshot={snapshot} onRefresh={() => void refresh()} onSelect={selectTask} /> : view === 'library' ? <LibraryView key={snapshot.project.id} snapshot={snapshot} onSelect={assetId => navigate('library', assetId)} /> : chatId ? <PlanningChatView key={chatId} chatId={chatId} snapshot={snapshot} onOpenNavigation={() => setSidebarOpen(true)} onNewChat={openCreate} creatingChat={creatingChat} onClose={closePlanningChat} onTaskified={task => navigate('tasks', task.id)} onOpenChat={id => navigate('planning-chat', id)} onOpenTask={selectTask} /> : null}</div>
      ...
      {documentOpen && libraryAssetId && !selected && <LibraryDocument key={snapshot.project.id} assetId={libraryAssetId} snapshot={snapshot} onClose={() => navigate('library')} onSelect={assetId => navigate('library', assetId)} onOpenTask={selectTask} />}
    </main>
