...
export function App() {
  ...
  const title = view === 'tasks' ? 'All tasks' : view === 'attention' ? 'Attention' : 'Chief of staff';
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebarOpen ? 'is-open' : ''}`}>
      ...
      <div className="sidebar-create"><Button variant="secondary" onClick={openCreate}><Plus size={15} /><span>Create task</span><kbd>C</kbd></Button></div>
    ...
    <main className="workspace-main">
      ...
      <div inert={!!selected} className={`page-content page-${view}`}>{view === 'tasks' ? <TaskList snapshot={snapshot} layout={layout} onSelect={selectTask} selectedId={selectedId || undefined} onCreate={openCreate} /> : view === 'attention' ? <AttentionView snapshot={snapshot} onSelect={selectTask} onRead={readAttention} /> : <ChiefView snapshot={snapshot} onRefresh={() => void refresh()} onSelect={selectTask} />}</div>
    ...
    {newTask && <TaskDialog key={parent?.id || 'new'} open={newTask} onOpenChange={setNewTask} snapshot={snapshot} parent={parent} onCreated={task => { void refresh(); setSelectedId(task.id); }} />}
