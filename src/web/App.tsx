...
import { AlertCircle, ArrowDownToLine, ArrowUpRight, Bell, BookOpen, Check, ChevronDown, ChevronRight, CircleDot, Command, Folder, LayoutGrid, List, Loader2, PanelLeft, Pause, Play, Plus, Settings2, Sparkles, X } from 'lucide-react';
...
function locationState(): { view: View; taskId: string | null; chatId: string | null; assetId: string | null } {
  const segments = window.location.pathname.split('/').filter(Boolean);
  const second = segments[1] ? decodeURIComponent(segments[1]) : null;
  if (segments[0] === 'attention') return { view: 'attention', taskId: null, chatId: null, assetId: null };
  if (segments[0] === 'chief') return { view: 'chief', taskId: null, chatId: null, assetId: null };
  if (segments[0] === 'library') return { view: 'library', taskId: null, chatId: null, assetId: second };
  if (segments[0] === 'planning-chats') return { view: 'planning-chat', taskId: null, chatId: second, assetId: null };
  if (segments[0] === 'tasks') return { view: 'tasks', taskId: second, chatId: null, assetId: null };
  return { view: 'tasks', taskId: null, chatId: null, assetId: null };
}
/** The second segment names the view's own record: a task, a planning chat, or a library file. */
function pathFor(view: View, id?: string | null): string {
  if (view === 'attention') return '/attention';
  if (view === 'chief') return '/chief';
  if (view === 'library') return id ? `/library/${encodeURIComponent(id)}` : '/library';
  if (view === 'planning-chat') return id ? `/planning-chats/${encodeURIComponent(id)}` : '/planning-chats';
...
export function App() {
  ...
  const navigate = useCallback((nextView: View, id: string | null = null) => {
    const path = pathFor(nextView, id);
  ...
  useEffect(() => {
    const handlePopState = () => { const next = locationState(); setView(next.view); setSelectedId(next.taskId); setChatId(next.chatId); setLibraryAssetId(next.assetId); setSidebarOpen(false); };
