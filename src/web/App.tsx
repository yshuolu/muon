...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace-main">
      ...
      {view !== 'planning-chat' && <div inert={!!selected} className="page-heading"><div><div className="page-heading-eyebrow"><span className="project-icon">{snapshot.project.name.slice(0, 1).toUpperCase()}</span>{snapshot.project.name}<span className="eyebrow-divider">/</span>Workspace</div><h1>{title}{view === 'attention' && unread > 0 && <span className="title-count">{unread}</span>}</h1><p>{view === 'tasks' ? 'A clear path from ideas to finished work.' : view === 'attention' ? 'The decisions that keep your work moving.' : 'A thoughtful partner for everything you’re building.'}</p></div><div className="page-actions">{view === 'tasks' && <div className="layout-toggle" aria-label="Task layout"><button aria-label="List view" aria-pressed={layout === 'list'} className={layout === 'list' ? 'active' : ''} onClick={() => setLayout('list')}><List size={15} /></button><button aria-label="Board view" aria-pressed={layout === 'board'} className={layout === 'board' ? 'active' : ''} onClick={() => setLayout('board')}><LayoutGrid size={14} /></button></div>}<Button onClick={() => void openCreate()} disabled={creatingChat}><Plus size={15} />New task</Button></div></div>}
      {!snapshot.project.repositoryPath && view !== 'planning-chat' && <div inert={!!selected} className="setup-banner"><span className="setup-banner-icon"><Folder size={18} /></span><div><strong>Connect your repository</strong><p>Develop tasks need a local Git repository. Brainstorm and Research tasks can start without one.</p></div><Button variant="secondary" size="sm" onClick={() => setSettings(true)}>Choose repository<ArrowUpRight size={13} /></Button></div>}
