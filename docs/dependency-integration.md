...
Only completed related tasks supply inputs. Dependencies on organizational groups expand recursively to their completed leaves, with duplicate sources removed. Develop dependencies supply Git patches; Brainstorm and Research dependencies supply their saved summaries and outputs without requiring a worktree. Source workspace identity, branch, and base are validated for code exports; a before/after content check rejects sources that change during export. The adapter never gains access to sibling working directories.

Each RFC stores its `dependencyInputs`, including source task identity, capture time, base/head commits, exact patch, changed-file list, and SHA-256 digest. Its `resultInputs` retain summaries and saved outputs with their own capture times and digests. Build and Verify reuse that approved RFC's saved inputs. External edits to a prerequisite after review do not change what the agent receives. Replanning captures a new set of inputs that requires a new owner approval. Older RFCs without required dependency snapshots must be replaced before integration can proceed.
