...
`AgentAdapter.run()` accepts an `AgentSessionRequest`: provider, session name, `SessionInput { systemPrompt, instructions, context }`, explicit access policy, absolute working directory, optional provider conversation ID, and cancellation signal. It returns final text and a provider conversation ID. A compatibility request with `phase` and `prompt` remains supported for existing chief/chat callers and tests. `available()` checks whether the selected executable answers `--version`; it does not prove authentication, account capacity, or sandbox readiness.

`SessionRunner` is the shared task-session execution path. Trusted server policy assigns each named session its repository and approval requirements and `read-only` or `workspace-write` access. Workflow parameters select the registered session sequence; they cannot grant write access or owner approval. Shared system instructions, session instructions, and task context remain distinct in the request. Claude receives the system prompt through its system-prompt option; Codex receives it as developer instructions.

Brainstorm and Research each use one agent session. Develop uses Plan, Build, and Verify, or Build and Verify when the server has validated an imported approved RFC. Every logical session starts a fresh provider conversation, while a Plan revision or explicit resume can continue that same logical session's conversation. Inputs and their digests are retained per attempt. Previous session outputs and approved dependency snapshots are supplied explicitly across session boundaries.
...
Plan and Brainstorm use Claude plan mode with Read, Glob, and Grep. Research adds WebSearch and WebFetch without allowing file writes or Bash. The chief uses default permission mode with Read, Glob, Grep, and Bash, but only its per-run Muon CLI executable is allowlisted. The required Bash sandbox denies project and launcher writes and allows only the Muon API loopback host/port. The CLI uses the sandbox HTTP proxy; no unsandboxed network fallback is allowed. A scoped server credential enforces task-management authority without owner approval authority. Tokens are never included in the prompt or provider arguments. With permission bypass disabled, Build and Verify use accept-edits mode with the Bash sandbox required, automatic permission only for sandboxed commands, and no unsandboxed retry. Restricted mode confines built-in file tools to working directories; MCP tools and configured hooks are disabled. Current Claude Code is required: `--restricted` was introduced in 2.1.248; live validation used 2.1.263. See the [CLI reference](https://code.claude.com/docs/en/cli-reference), [permission modes](https://code.claude.com/docs/en/permission-modes), and [sandbox configuration](https://code.claude.com/docs/en/sandboxing).
...
Build and Verify run with full local permissions by default: Claude uses `--dangerously-skip-permissions` with its OS sandbox disabled, and Codex uses `approvalPolicy: never` with `danger-full-access`. Set `MUON_AGENT_BYPASS_PERMISSIONS=0` to restore their restricted policy, where `MUON_CLAUDE_ALLOWED_DOMAINS` and `MUON_CLAUDE_ALLOW_LOCAL_SERVERS=1` control Claude's network access. Named read-only sessions remain read-only even when that setting is enabled. Compatibility phase/prompt calls retain their prior permission behavior. The chief retains its separate scoped CLI-only command policy. Full access is an explicit local-owner policy and can reach outside isolated task worktrees.
...
Plan, Brainstorm, Research, and chief runs receive a read-only sandbox. Research enables provider web search. With permission bypass disabled, Build and Verify receive workspace-write permission for the task directory with outbound network enabled for dependency installation and verification. Approval policy is `never`; unexpected permission or interactive requests fail with an attention error. `turn/completed` must report success. Final agent messages are selected by `final_answer`; older messages without phase metadata are supported. Plan accepts a completed plan item. Reasoning, commentary, tool calls, deltas, and tool output are ignored. The wire behavior was checked against the actual CLI's generated protocol and the official [Codex App Server documentation](https://learn.chatgpt.com/docs/app-server). Thread sandbox enums use `read-only`/`workspace-write`, while turn sandbox policy types use `readOnly`/`workspaceWrite`.
...
Each Develop task gets a Muon-owned Git worktree before its first session. Plan, approved Build, and Verify reuse that checkout and branch. An imported approved RFC retains its original base commit for the new task's worktree. Brainstorm and Research use the configured repository with read-only access, or a managed scratch directory when none is configured. The workspace is independent of provider conversations, so starting another session does not discard edits. Claude's native `--worktree` support would create a provider-specific lifecycle; Muon avoids depending on it and does not ask either agent to create another worktree.
