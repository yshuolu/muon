...
Chief requests may supply a `model` override from the project's saved `chiefModel` setting; disposable planning chats supply their own selected `model`. The Claude adapter honors overrides in the chief and chat phases, retaining its configured model when the override is absent. Task planning, building, discussion, and verification keep the configured model. Thinking effort stays configured for every phase. Model selection preserves the chief's scoped CLI permissions and the configured permission policy for chats.
...
Muon task agents run with full local permissions by default: Claude uses `--dangerously-skip-permissions` with its OS sandbox disabled, and Codex uses `approvalPolicy: never` with `danger-full-access`. Set `MUON_AGENT_BYPASS_PERMISSIONS=0` to restore the restricted policy, where `MUON_CLAUDE_ALLOWED_DOMAINS` and `MUON_CLAUDE_ALLOW_LOCAL_SERVERS=1` control Claude's network access. The chief retains its separate scoped CLI-only command policy. Full access is an explicit local-owner policy and can reach outside isolated task worktrees.
