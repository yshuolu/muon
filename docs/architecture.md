...
The chief agent is a per-project setting. Claude Code enforces the boundary with a Bash allowlist for the launcher, denied source writes, denied `.muon` and `.env` reads, and a proxied loopback-only network. Codex enforces it structurally: the chief runs from an empty scratch directory under Codex's workspace-write sandbox with network access, so the repository is readable but not writable, the launcher can reach the loopback API, and approval requests are declined; it never receives full access even when task agents do. Codex offers no per-path read denials, so a Codex chief can read repository secrets that the Claude chief cannot. The API grants the chief task reads, creation, eligible edits, cancellation, and recovery. It rejects RFC approvals, owner review feedback, settings, attention acknowledgements, and recursive chief requests. Unknown, expired, and canceled tokens never fall back to owner authority. The local owner UI/CLI still trusts loopback access on this machine; this is not a hostile-process or multiuser isolation boundary. Cloud hosting must supply authenticated identities, membership checks, request-specific scopes, and its deployment boundary.
