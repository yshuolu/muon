...
The chief agent is a per-project setting. The chief and planning chats are advisory sessions in the owner's real repository, not task worktrees, so both adapters exclude them from the full-access policy and confine writes to a per-run scratch directory that is deleted afterwards. Claude Code enforces the boundary with denied `Edit`/`Write` tools, a sandbox whose only writable path is the scratch directory, denied `.muon` and `.env` reads, a Bash allowlist for the chief's launcher with a proxied loopback-only network, and sandboxed Bash without network for chats. Codex enforces it structurally: the session runs from the scratch directory under Codex's workspace-write sandbox, so the repository is readable but not writable and approval requests are declined; the chief has network access so the launcher can reach the loopback API, while chats have none. Codex's network switch is all-or-nothing and it offers no per-path read denials, so a Codex chief can reach other hosts and read repository secrets that the Claude chief cannot; its sandbox also always permits writes under `/tmp` and `$TMPDIR`, so repositories kept there are not protected. Claude Code runs advisory sessions with the scratch directory as the working directory, so the CLI's own state files never appear in the repository. The API grants the chief task reads, creation, eligible edits, cancellation, and recovery. It rejects RFC approvals, owner review feedback, settings, attention acknowledgements, and recursive chief requests. Unknown, expired, and canceled tokens never fall back to owner authority. The local owner UI/CLI still trusts loopback access on this machine; this is not a hostile-process or multiuser isolation boundary. Cloud hosting must supply authenticated identities, membership checks, request-specific scopes, and its deployment boundary.
...
Composition is confined to `src/server/index.ts`. The fixed workspace and owner identity is supplied there rather than accepted from HTTP request bodies. `ProjectRegistry` (`src/server/project-registry.ts`) loads every project row for that workspace, seeds the first project when none exists, and runs one `TaskService` coordinator per active project over the shared repository, asset storage, worktree provider, and adapters; provider availability is probed once and shared. The HTTP layer resolves the coordinator from the `/api/projects/:project` path segment (or the default project for unprefixed routes) and the chief gateway binds each credential to the project that opened it. Archiving stops a coordinator without deleting records; restoring starts a new one. A future authenticated HTTP layer must resolve the workspace and user per request and authorize project membership before this resolution. The application already separates owner and delegated provider and validates RFC owner authority.

The repository stores task aggregates in JSON payloads with relational scope, identity, status, priority, and version columns for keys and indexes. Assets have their own scoped table, with storage identity columns and metadata payloads. Replacing SQLite with PostgreSQL can preserve the port and use JSONB; it still requires migrations, transactional equivalence, and operational work. Schema version 2 adds the asset table, version 3 the planning chats, and version 4 the review comments on Library documents (with a unique request ID per document for idempotent submission and an indexed status for pending counts), each through a transactional upgrade; databases from a newer version are rejected. Documents stay immutable: resolving comments that ask for changes stores a new asset whose `previousVersionId` links the lineage, while comments and replies stay on the version they were made on.
