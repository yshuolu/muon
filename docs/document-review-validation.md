...
Readers now always show the newest version: the asset service marks a revised version with `latestVersionId` (covered by the HTTP test, which checks both the single read and the listing after a resolve run), the Library list hides superseded versions, a tab that loads a superseded version swaps to the newest one (tab helper test), and a finished review replaces the reviewed document in its tab instead of offering banners. The resolved thread is expanded on the new version.
