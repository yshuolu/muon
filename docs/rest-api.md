...
| `PATCH /tasks/:task` | `EditTaskRequest` | 200 `Task`; scope edits after planning started queue a fresh RFC (see below) |
...
| `PATCH /planning-chats/:id` | `{ "model": "sonnet" }`, `{ "model": null }`, `{ "provider": "codex" }`, or `{ "effort": "high" }` | 200 planning chat with its selected agent, model, and thinking effort |
...
Task edits follow the RFC gate rather than a single "unstarted only" rule. `title` and `description` may change until the latest RFC is approved; when the task has already started, the edit stops a running planning run (its slot is released and the run is not recorded as a failure) or marks a pending RFC `changes_requested`, clears the task's attention items, and requeues planning so the next RFC reflects the new scope. Once the RFC is approved, scope edits return 400 and changes go through a replan comment or a replan retry. `priority`, `labels`, `effort`, `parentId`, and `blockedByIds` may change until the task is `done` or `canceled`; `provider` and the `backlog`/`todo` status only while the task is unstarted (`backlog` or `todo` with phase `idle`). A canceled task still accepts `parentId: null`. Groups may be edited until canceled.

Settings accepts optional `maxConcurrentAgents` (integer 1–8), `dispatcherEnabled` (boolean), `defaultProvider`, `repositoryPath`, `projectName`, `chiefProvider`, `chiefModel`, `chiefEffort`, and `chiefSoul`. `chiefProvider` is `claude` or `codex` (`null` means Claude Code); changing it clears `chiefModel` and `chiefEffort` unless the same request sets them, and assistant chief messages record the `provider` that produced them. `chiefEffort` is a thinking effort level the chief's agent accepts (`null` for the configured default; a level of the other agent is stored as `null`) and, like the model, returns 409 while the chief is running. Tasks carry the same optional `effort` (`CreateTaskRequest` and `PATCH /tasks/:task`), applied to every run of that task; a level the task's agent does not accept is ignored at run time. The chief model is a trimmed model alias or identifier of 1–200 characters, starting with a letter or digit and containing only letters, digits, `.`, `_`, `:`, `/`, `[`, `]`, or `-`. Set it to `null` to use the chosen agent's configured default. `chiefSoul` is trimmed text up to 20,000 characters; set it to `null` to restore the default behavior. It is included as owner-authored persona and communication context in future Chief prompts, while Muon's permissions and approval rules remain authoritative. Model and SOUL changes return 409 while a chief request is queued or running. Settings writes and chief submission are serialized so an accepted request retains its selected configuration. Model availability is checked by the Claude runtime when it runs. Repository changes require a valid committed Git root and are rejected while they would disrupt current work. Changing project display name does not change its stable project ID or task identifier prefix.
...
Each planning chat includes `model`, initially `null` for the configured Claude default, and `effort`, initially `null` for the configured thinking effort. The owner may change them using `PATCH /planning-chats/:id`; model identifiers follow the same validation as `chiefModel`, and `effort` must be one of the levels the chat's provider accepts (`low`, `medium`, `high`, `max` for Claude Code; `low`, `medium`, `high`, `xhigh`, `ultra` for Codex), else it is stored as `null`. Switching the provider resets both. Changes return 409 while a reply is being admitted or running. Every planning reply and chief run receives read-only copies of the project's current Markdown and text Library documents (newest version of each, up to 2 MB per document, 200 documents, and 32 MB in total) in a `library` folder of its scratch directory, listed in the prompt with their asset IDs; `[name](asset://ID)` references in messages point the agent at those copies. The selection applies to subsequent replies in that chat, including after a provider failure, and lasts until the chat is discarded or the server restarts. It does not change chief settings or task execution models. Model availability is checked by Claude when the next reply runs.
