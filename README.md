...
Planning chats use a compact toolbar for navigation and **Taskify**, leaving the rest of the view for the conversation. The partner never does the work itself: when you ask for implementation, commits, or setup, it answers with a plan and a **Proposed task** block, and Taskify opens prefilled with that title and description. When you ask it for documents, it publishes them to the Library and links them in its reply. Chats are saved in the local database, so they survive a server restart; a reply that was in flight during a restart is marked interrupted, and sending a follow-up continues the conversation. Chats disappear only when discarded, when Taskify turns them into a task, or when an empty chat is cleaned up at startup. If an old chat is no longer available, choose **Start new chat** on that screen or **Create task** in the sidebar to open a fresh conversation.
