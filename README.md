...
Choose the quick planning chat's agent and model from the dropdowns in its message toolbar. The agent is **Claude Code** or **Codex**; both answer read-only in the project repository, and an agent that is not detected is labeled as such. The model dropdown offers the agent's configured default plus, for Claude Code, `opus`, `sonnet`, and `haiku`, and **Enter model ID…** for another model of that agent. Switching agents returns the model to that agent's default. Selections save immediately for that disposable chat and survive page reloads while the server is running. After a usage-limit error, choose another available model or agent and send a follow-up to continue with the same conversation. Wait for an active reply to finish before switching. The **(default)** option restores `MUON_CLAUDE_MODEL` or `MUON_CODEX_MODEL`.
